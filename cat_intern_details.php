...
<div class="container">
	...
	<div id="contents" class="row">
        ...
        <div id="alljobs_leftside" class="col-md-3 col-sm-3 hidden-xs">
        	<h3>INTERNSHIPS BY <span style="color: #F00;">CATEGORIES</span></h3>
                        ...
                        <?php
			  			...
			  			?>		
							<a href='cat_intern_details.php?cat_id=<?php echo $cat_id; ?>'>
							<li>
 	     	                      <?php echo $cat_name; ?>
    	    	                  <?php echo "<span style='color: black;'>(" . countCategoryIntern($cat_id) . ")</span><br/>"; ?>
    	...
    	<div id="all_jobs" class="col-md-9 col-sm-9 col-xs-12">
        	...
        	<?php					
					if(isset($_GET['cat_id'])){
						...
						$statement = "internship WHERE intern_cat = '$cat_id' ORDER BY intern_post_date DESC"; // Change `records` according to your table name.
  ...
  //      				$get_cat_job = "SELECT * FROM jobs WHERE cat_id = '$cat_id'";
	//					$run_cat_job = mysqli_query($connection, $get_cat_job);
					...
					while($row_cat_job = mysqli_fetch_array($results)){
						$job_id = $row_cat_job['intern_id'];
						$job_title = $row_cat_job['intern_title'];
						$job_post_date = date("d F Y", strtotime($row_cat_job['intern_post_date']));
						$company = $row_cat_job['intern_comp'];
						$job_location = $row_cat_job['intern_city'];
						$job_desc = substr($row_cat_job['intern_desc'],0,150);
			...
			?>
				<div id="category_all_jobs" class="row">
					...
					<h4>
						<?php
							echo $company;
							if($job_location == 0){
								echo "<span>&nbsp;-&nbsp;Multiple Cities</span>";
							}
							else{
								echo "<span>&nbsp;-&nbsp;{$city_title}</span>";
							}
						?>
					</h4>
					<h2>Posted: <?php echo $job_post_date; ?></h2>
					<h5><?php echo $job_desc; ?><a href="intern_details.php?intern_id=<?php echo $job_id; ?>">&nbsp;&nbsp;View Details</a></h5>
				</div>
        		...
        		<?php
        			echo intern_cat_pagination($statement,$per_page,$page,$url='?',$cat_id);
				?>
        		</div>
        </div>
